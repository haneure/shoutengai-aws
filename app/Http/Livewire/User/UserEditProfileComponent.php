    ...
    public function updateProfile() {
        ...
        if($this->newimage) {
            ...
            if(strpos($this->image, 'http') !== false){
                $image_format = explode('/', $this->image);
                $image_format = end($image_format);
                Storage::disk('s3')->delete('profile/'.$image_format);
            } else {
                unlink('assets/images/profile'.'/'.$this->image);
            }
            ...
            //add to s3
            $path = $this->newimage->storeAs('profile', $imageName, 's3');
            Storage::disk('s3')->setVisibility($path, 'public');
            $user->profile->image = Storage::disk('s3')->url($path);
        }
        ...
        $user->profile->save();
        session()->flash('message', 'Profile has been updated successfullly!');
    }
